...
REM ВАЖЛИВО: використовуємо Python з .venv, а не глобальний
set "PYTHON_PATH=%~dp0.venv\Scripts\python.exe"

REM Якщо треба, можеш змінити порт API тут
...
if %errorlevel%==0 (
    ...
    if %errorlevel% neq 0 (
        echo [ПОМИЛКА] Не вдалося запустити PostgreSQL.
        pause
        exit /b 1
    )
)
...
for /L %%i in (1,1,15) do (
    "%PG_PATH%\bin\psql.exe" -U postgres -d hero_manager -c "SELECT 1;" >nul 2>&1
    if not errorlevel 1 (
        set "DB_READY=1"
        goto :db_ready
    )
