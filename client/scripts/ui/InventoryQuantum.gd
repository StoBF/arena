...
func _populate_resources():
    resources_grid.clear()
    for name in resources.keys():
        var qty = resources[name]
        var h = HBoxContainer.new()
        var icon = TextureRect.new()
        icon.texture = preload("res://assets/icons/resource_placeholder.png")
        icon.rect_min_size = Vector2(32,32)
        h.add_child(icon)
        var l = Label.new()
        l.text = "%s: %d" % [name, qty]
        h.add_child(l)
        h.connect("mouse_entered", Callable(self, "_on_resource_hover")).bind(name)
        h.connect("mouse_exited", Callable(self, "_on_tooltip_hide"))
...
func _populate_recipes():
    ...
    for recipe in recipes:
        ...
        v.connect("gui_input", Callable(self, "_on_recipe_gui_input")).bind(recipe)
        v.connect("mouse_entered", Callable(self, "_on_recipe_hover")).bind(recipe)
        v.connect("mouse_exited", Callable(self, "_on_tooltip_hide"))
        recipes_grid.add_child(v)

func _on_recipe_gui_input(event, recipe):
    if event is InputEventMouseButton and event.pressed and event.button_index == BUTTON_LEFT:
        selected_recipe = recipe
        emit_signal("recipe_selected", recipe.id)
...
func _has_resources_for_recipe(recipe):
    for name in recipe.requirements.keys():
        if resources.get(name, 0) < recipe.requirements[name]:
            return false
    return true
...
func _on_craft_pressed():
    ...
    # deduct resources
    for name in selected_recipe.requirements.keys():
        resources[name] -= selected_recipe.requirements[name]
    _populate_resources()
    ...
    if equipment.mutation_chance > 0 and randi() / float(RAND_MAX) < equipment.mutation_chance:
        ...
        # additional strength/duration could be added here
    crafted_equipment.append(equipment)

    emit_signal("craft_pressed", selected_recipe.id)
    _update_craft_button()

func _on_recipe_hover(recipe):
    var text = "%s\nRequirements:" % recipe.name
    for name in recipe.requirements.keys():
        text += "\n - %s x%d" % [name, recipe.requirements[name]]
    _show_tooltip(text)

func _on_resource_hover(name):
    var qty = resources.get(name, 0)
    _show_tooltip("%s: %d in warehouse" % [name, qty])

func _show_tooltip(text):
    if tooltip:
