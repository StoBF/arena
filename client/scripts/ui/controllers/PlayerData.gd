...
func _load_data() -> void:
	...
	if parsed is Dictionary:
		var data := _normalize_hero_payload(parsed as Dictionary)
		heroes.clear()
		for hero_variant in (data.get("heroes", []) as Array):
			heroes.append(UIModels.hero(hero_variant as Dictionary))
		resources = UIModels.resource_map((data.get("resources", {}) as Dictionary))
	if heroes.size() > 0:
...
func get_heroes() -> Array:
	var result: Array = []
	for hero in heroes:
		result.append(UIModels.hero(hero as Dictionary))
	return result

func get_resources() -> Dictionary:
	return UIModels.resource_map(resources)

func get_selected_hero() -> Dictionary:
	for hero in heroes:
		var entry := hero as Dictionary
		if str(entry.get("id", "")) == selected_hero_id:
			return UIModels.hero(entry)
	return {}
...
func select_hero_by_index(index: int) -> void:
	...
	hero_selected.emit(UIModels.hero(hero))
...
func create_hero(hero_name: String) -> bool:
	...
	var hero := UIModels.hero({"id": id, "name": hero_name.strip_edges()})
	heroes.append(hero)
	selected_hero_id = id
	heroes_changed.emit(get_heroes())
	hero_selected.emit(UIModels.hero(hero))
...
func consume_resources(requirements: Dictionary) -> bool:
	if can_consume_resources(requirements) == false:
		return false
	var normalized_requirements := UIModels.resource_map(requirements)
	for key in normalized_requirements.keys():
		var k := str(key)
		resources[k] = int(resources.get(k, 0)) - int(normalized_requirements[key])
	resources_changed.emit(get_resources())
	return true

func _normalize_hero_payload(payload: Dictionary) -> Dictionary:
	var schema_version: int = int(payload.get("schema_version", 0))
	if schema_version <= 0:
		return {
			"schema_version": SUPPORTED_SCHEMA_VERSION,
			"heroes": (payload.get("heroes", []) as Array).duplicate(true),
			"resources": UIModels.resource_map(payload.get("resources", {}) as Dictionary),
		}

	if schema_version != SUPPORTED_SCHEMA_VERSION:
		push_warning("[PlayerData] Unsupported schema_version=%d. Applying safe defaults." % schema_version)
