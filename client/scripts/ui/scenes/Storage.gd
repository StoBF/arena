...
func _on_delete_hero() -> void:
        ...
        var hero_name: String = _selected_hero_name()
        var confirmed: bool = await UIUtils.confirm_async(
                "Delete hero \"%s\"?\nThis action is permanent." % hero_name
        )
        if not confirmed:
                delete_hero_button.disabled = false
                return
        var response: Dictionary = await ApiClient.request_delete("/heroes/%d" % _selected_hero_id)
        if not bool(response.get("ok", false)):
                var err: String = str(response.get("message", response.get("error", "Failed to delete hero")))
                UIUtils.show_error(err)
                delete_hero_button.disabled = false
                return
        UIUtils.show_success("Hero \"%s\" deleted." % hero_name)
        AppState.current_hero_id = 0
        _selected_hero_id = -1
