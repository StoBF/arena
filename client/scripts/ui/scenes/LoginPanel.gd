...
func _localize_ui() -> void:
	sign_in_button.text = tr("ui.common.login")
	create_button.text = tr("ui.common.register")
	username_field.placeholder_text = tr("ui.common.email")
	password_field.placeholder_text = tr("ui.common.password")
	if remember_me_checkbox:
		remember_me_checkbox.text = tr("ui.login.remember_me")
	if language_label:
	...
	if server_status_refresh_btn:
		server_status_refresh_btn.text = tr("ui.common.refresh")

func _setup_server_status() -> void:
	if server_status_label:
		server_status_label.text = tr("ui.login.server_checking")
	if server_status_refresh_btn:
...
func _on_server_status_checked(online: bool, latency_ms: float, error_message: String) -> void:
	if not is_instance_valid(server_status_label):
		return
	if online:
		server_status_label.text = tr("ui.login.server_online") + " (" + (tr("ui.login.server_latency") % int(latency_ms)) + ")"
	else:
		server_status_label.text = tr("ui.login.server_offline") + " — " + error_message
