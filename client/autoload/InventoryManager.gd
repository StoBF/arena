...
## Map from canonical client slot names → server SlotType enum values.
## The server uses its own domain-specific names (spacesuit, utility_belt, artifact).
const _CLIENT_TO_SERVER_SLOT: Dictionary = {
        "Helmet": "helmet",
        "Armor": "spacesuit",
        "Weapon": "weapon",
        "Boots": "boots",
...
func equip_item(hero_id: int, item_id: int, slot_type: String) -> bool:
        ...
        var server_slot: String = _CLIENT_TO_SERVER_SLOT.get(slot_type, slot_type.to_lower())
        var response: Dictionary = await _perform_request(HTTPClient.METHOD_POST, "/equipment/", {
                "hero_id": hero_id,
                "item_id": item_id,
                "slot": server_slot,
        })
...
func unequip_item(hero_id: int, slot_type: String) -> bool:
        ...
        # Fetch the equipment list to get the record ID for this slot
        var eq_response: Dictionary = await _perform_request(HTTPClient.METHOD_GET, "/equipment/hero/%d" % hero_id)
        if not bool(eq_response.get("ok", false)):
                return false

        var eq_data: Variant = eq_response.get("data", {})
        var eq_list: Array = []
        if eq_data is Dictionary:
                var d := eq_data as Dictionary
                if d.has("result") and d["result"] is Array:
                        eq_list = d["result"] as Array
                elif d.has("items") and d["items"] is Array:
                        eq_list = d["items"] as Array
        elif eq_data is Array:
                eq_list = eq_data as Array

        var equipment_id: int = -1
        for entry_variant: Variant in eq_list:
                if entry_variant is Dictionary:
                        var entry := entry_variant as Dictionary
                        var entry_slot: String = _normalize_slot_name(str(entry.get("slot", "")))
                        if entry_slot == slot_type:
                                equipment_id = int(entry.get("id", -1))
                                break

        if equipment_id <= 0:
                _set_error("not_found", "No equipped item found in slot '%s'" % slot_type)
                return false

        var response: Dictionary = await _perform_request(HTTPClient.METHOD_DELETE, "/equipment/%d" % equipment_id)
...
func sell_item_on_auction(item_id: int, price: float = -1.0) -> bool:
        ...
        var response: Dictionary = await _perform_request(HTTPClient.METHOD_POST, "/auctions/", {
                "item_id": item_id,
                "start_price": price,
                "duration": 24,
                "quantity": 1,
